...
//        when(vehicleRepository.findByStatus(status)).thenReturn(vehicles);
        when(vehicleRepository.findByStatusWithDetails(status)).thenReturn(vehicles);

        // When
        List<Vehicle> result = vehicleService.getVehiclesByStatus(status, true);

        // Then
        assertEquals(2, result.size());
        verify(vehicleRepository).findByStatusWithDetails(status);
    }
